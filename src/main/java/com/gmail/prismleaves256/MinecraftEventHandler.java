...
import com.gmail.prismleaves256.event.OpenChestEvent;
import com.gmail.prismleaves256.util.HypixelEntityExtractor;
import com.gmail.prismleaves256.wrapper.MinecraftSound;
...
public class MinecraftEventHandler {
	...
	private HashSet<Integer> oldArmorStand = new HashSet<>();
	...
	public void onWorldRender(RenderWorldLastEvent event) {
		...
		for(Entity e:entities){
			if(e instanceof EntityArmorStand){
				if(!oldArmorStand.contains(e.getEntityId())){
					oldArmorStand.add(e.getEntityId());
					DamageDatas.add((EntityArmorStand)e);
				}
			}
		}
		DamageDatas.draw(event.partialTicks);
	}
}